...
public:
  KissBroker(std::string device, int baud, std::string socket_dir,
             ...
             std::string rf_endpoint_name = "repeater");
  ~KissBroker();
...
private:
  ...
  void deliverLocalTx(size_t sender, const std::vector<uint8_t>& encoded);
  ...
  std::string rf_endpoint_name_;
  size_t rf_endpoint_ = 0;
  int physical_fd_ = -1;
